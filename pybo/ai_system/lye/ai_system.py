...
class AiSystem:
    ...
    def process_image(self, image_path, target_encodings):
        ...
        """
        try:
            print('===LYE:AiSystem def process-image=============================')
            ...
            print('==================================================================')
            print('self._detect_faces(image_path)를 image_rgb, faces로 나눔.')
            print('image_rgb ==>', image_rgb)
            print('faces     ==>', faces)
            print('===LYE:End:AiSystem:process-image=============================')
    ...
    def _detect_faces(self, image_path):
        ...
        try:
            ...
            print('===============(4)LYE:AiSystem Printing================')
            print('self.detector_manager : checking--------------------------------------')
            print('self.detector_manager 는 wrapper의 detetor_manager 객체와 같아야 합니다.')
...
class ForDjango(AiSystem):
    def __init__(self, config, detector_manager, predictor_manager):
        ...
        """
        print('(3)ForDjango__config,detector_manager,predictor_manager 받음.______________불러지기만 함.')
        print('여기는 ForDjango의 init 생성자이다. Wrapper에게 받은 변수들을 확인해보자')
        print('config           :', config)
        print('detector manager :', detector_manager)
        print('predictor_manager:', predictor_manager)
        #
    #
    def process_image(self, image_path, target_encodings):
        """
        ...
        """
        try:
            print('(3)ForDjango:LYE:process_image start===============================')
            print('image_path =>',image_path)
            print('target_encodings =>',target_encodings)
            image_rgb, faces = self._detect_faces(image_path)  # 얼굴 탐지
            print('self._detect_faces(image_path)',self._detect_faces(image_path))
            print('image_rgb => ',image_rgb)
            print('faces 변수 =>',faces)
            if self.predictor_manager:  # 예측기가 설정된 경우
                predictions, face_cnt, race_cnt, male_cnt = self._complicate_predictions(image_rgb, faces, target_encodings)  # 얼굴 예측
            else:
                print('\n안녕하세요, else문입니다.\nprocess_image:AiSystem 다녀오는 길입니다.')
                predictions, face_cnt, race_cnt, male_cnt = faces, f'{len(faces)}', None, None
                print('predictions',predictions)
                print('face_cnt',face_cnt)
                print('race_cnt',race_cnt)
                print('male_cnt',male_cnt)
            result_image = self._draw_results(image_rgb, predictions, face_cnt, male_cnt, race_cnt)  # 결과 그리기
...
class DetectorManager:
    ...
    def load_detectors(selected_detectors, config):
        ...
        print('=============(1)LYE:This DetectorManager Printing===========\n탐색기 선택됨.탐색기 선택됨.탐색기 선택됨.탐색기 선택됨.')
        print('FaceDetector(*detectors): 객체 checking----------------\n그냥 호출된 =>(1)',FaceDetectors(*detectors))
        print('=============(1)LYE:End:DetectorManager End===========')
        return FaceDetectors(*detectors)
...
def setup_system(func):
    ...
    def wrapper(request, image_path, *args, **kwargs):
        ...
        print('==========(2)LYE:Wrapper Printing ==============')
        print("DetectorManager.load_detectors(selected_detectors, config)")
        print("-------------------\n=첫번째 정의된 detector_manager ==> :", detector_manager)
